...
```
implementation 'com.github.Foysalofficial:sliding-root-nav:2.0'
```
...
## Sample
Please see the [sample app](https://github.com/Foysalofficial/sliding-root-nav/tree/master/app) for a library usage example.
...
new SlidingRootNavBuilder(this)
  ...
  .inject();
```
`customTransformation` in the above example is a user-created class that implements `RootTransformation` interface. For an example, refer to the [default transformations](library/src/main/java/com.foysaldev.slidingrootnav.transform). 
